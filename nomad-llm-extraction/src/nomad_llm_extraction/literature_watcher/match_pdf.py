...
from utils import download_pdf, get_pdf_url
...
def check_pdfs():
    ...
    print_stats(stats)
    old_found_urls = (
        json.load(open(f'{papersbot_runs_path}/found_pdf_urls.json'))
...
def download_pdfs(download_dir: str | Path = 'downloaded_papers') -> list[Path]:
    ...
    try:
        with open(f'{papersbot_runs_path}/found_pdf_urls.json') as f:
