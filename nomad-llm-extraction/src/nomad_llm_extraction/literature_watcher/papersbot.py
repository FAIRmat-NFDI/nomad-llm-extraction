...
import feedparser
from configuration import RELAXED_REGEX, STRICT_REGEX, papersbot_runs_path
from loguru import logger
from match_pdf import check_matches, check_pdfs, download_pdfs
from proc_abstracts import check_relaxed_match_doi, get_abstracts, update_for_retry
from tqdm import tqdm
from utils import get_doi, save_summaries
...
def read_feeds_list():
    with open(files('perla_extract').joinpath('papersbot/feeds.txt')) as f:
...
def read_posted():
    try:
        with open(f'{papersbot_runs_path}/posted.dat') as f:
