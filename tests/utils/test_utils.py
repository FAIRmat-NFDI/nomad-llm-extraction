...
def test_get_repo_metadata_parses_github_remotes(monkeypatch, remote_url, expected_url):
    ...
    monkeypatch.setattr(
        utils.subprocess, 'check_output', lambda *args, **kwargs: next(responses)
    )

    assert utils.get_repo_metadata() == ('abc123', expected_url)
...
def test_get_repo_metadata_returns_none_when_git_is_unavailable(monkeypatch, error):
    ...
    monkeypatch.setattr(utils.subprocess, 'check_output', raise_error)

    assert utils.get_repo_metadata() == (None, None)
