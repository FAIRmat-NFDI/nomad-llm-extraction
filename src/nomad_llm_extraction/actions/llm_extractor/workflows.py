    ...
    async def run(self, data: ExtractionWorkflowInput) -> dict:
        ...
        try:
            ...
            for pdf, pdf_text, doi in list_of_pdfs['texts']:
                ...
                single_extraction_input.extraction_metadata['doi'] = doi
                extraction_result = await workflow.execute_child_workflow(
                    ExtractTextWorkflow.run,
            ...
            workflow.logger.exception(f'Unexpected exception in ExtractPDFWorkflow: {e}')
            raise ApplicationError(
...
class ExtractTextWorkflow:
    ...
    async def run(self, data: ExtractionWorkflowInput) -> dict:
        ...
        try:
            ...
            workflow.logger.exception(f'Unexpected exception in ExtractTextWorkflow: {e}')
            raise ApplicationError(
