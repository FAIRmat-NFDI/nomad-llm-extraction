...
import torch
from src.paths import DB_PATH, CHUNKS_DIR, DATA_DIR, LOGS_DIR
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOGS_DIR / 'embedding.log'),
    ...
    def load_chunks_to_db(self, chunk_file_path: Path):
        ...
        try:
            ...
            for chunk in chunks:
                ...
                token_ids = self.model.tokenizer.encode(text_content, add_special_tokens=False)
                ...
                if total_tokens <= SAFE_TOKEN_LIMIT:
                    ...
                    logger.info(f"✂️ Splitting massive chunk {base_id} ({total_tokens} tokens) into pieces...")
                    ...
                    while start < total_tokens:
                        ...
                        sub_text = self.model.tokenizer.decode(chunk_token_ids, skip_special_tokens=True)
            ...
            logger.info(f"Processed {chunk_file_path.name} -> Generated {count} vectors.")
            return 1
...
def main():
    ...
    cursor.execute("SELECT paper_id FROM papers WHERE parsing_status IN ('success_grobid', 'success_chunked', 'success_nougat')")
